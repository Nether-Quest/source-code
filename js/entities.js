...
Player = function(){
	...
	self.onDeath = function(){
		...
		alert("You died! You survived for " + timeSurvived + " ms. Click ok to go back to homescreen.")
		self.hp = 1
		open("/index.html")	
	}
...
Actor = function(type,id,x,y,width,height,img,hp,atkSpd){
	...
	self.updatePosition = function(){
		...
		if(self.x < self.width/2)
			self.x = self.width/2;
		if(self.x > Maps.current.width-self.width/2 + 2570)
			self.x = Maps.current.width - self.width/2 + 2570;
		if(self.y < self.height/2)
			self.y = self.height/2;
		if(self.y > Maps.current.height - self.height/2 + 887)
			self.y = Maps.current.height - self.height/2 + 887;
